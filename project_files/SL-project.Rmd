...
The comparison that takes into account all classifiers is at the end of the document, but for now, we can notice that the LDA preformed worse than the logistic classifiers with respect to the overall accuracy. Something that might be crucial in this aspect is that, as we saw during the fitting stage, the assumptions of the LDA were not fulfilled by our data. Moreover, we observe that the additional variables that the "fuller" model incorporates (`highest_education` and `imd_band_middle`) do not improve the classifier's performance. On the other hand, this model seems to be better at predicting 'Successful' examples than 'Unsuccessful' observations, since the sensitivity is above 0.8 and the specificity is below 0.7 in both cases. 
...
The QDA obtained results with the same patterns as those of the LDA, especially regarding the overall accuracy. However, we observe that the difference between sensitivity and specificity is larger in the case of the QDA. Once again, a possible cause of the decrease in the overall performance, compared to the logistic classifiers, is that our data does not meet the assumptions of the QDA model, even if in this case the condition that the variance is the same for the two groups was relaxed. Additionally, as before, the "fuller" QDA did not outperform the version with just the `log_clicks` predictor.
...
- <https://www.warrington.gov.uk/sites/default/files/2019-09/english-indices-of-deprivation-2015-research-report.pdf>
- <https://www.statlearning.com/>
- <https://www.ibm.com/support/pages/unexpected-sign-regression-coefficients>
